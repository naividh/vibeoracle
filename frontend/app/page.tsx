    ...
    return (
          <div className="flex items-end gap-0.5 h-8">
            {data.map((val, i) => {
                    ...
                    return (
                                <div key={i} className={`w-1.5 ${color} rounded-sm opacity-${60 + i * 10}`} style={{ height: `${Math.max(height, 10)}%` }} />
                              );
          })}
          </div>div>
        );
...
function TokenCard({ token, onClick }: { token: Token; onClick: () => void }) {
    ...
    return (
          <div 
                  onClick={onClick}
                  className="bg-[#12121a] rounded-xl p-5 border border-gray-800 hover:border-orange-500/50 transition-all cursor-pointer hover:scale-[1.02]"
                >
...
export default function Home() {
    ...
    const filteredTokens = tokens
          .filter(t => {
                  if (filter === "bullish") return t.score > 0;
                  if (filter === "bearish") return t.score < 0;
                  return true;
          })
          .filter(t => 
                  search === "" || 
          ...
          <main className="min-h-screen bg-[#0a0a0f] text-white">
                ...
                <section className="px-6 py-12 text-center">
                        ...
                        <div className="flex items-center justify-center gap-4 mt-6">
                                  <span className="text-xs px-3 py-1 rounded-full bg-blue-500/20 text-blue-400 border border-blue-500/30 flex items-center gap-1">
                                              <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg>svg>
                                              Built on Base
                                  </span>span>
